...
class Decoder2(nn.Module):
	def __init__(self):
		...
		self.deconv4 = nn.Sequential(
			nn.ConvTranspose3d(self.nff[5], self.nfb[0], kernel_size=2, stride=2,output_padding=(0,1,1)),
			nn.InstanceNorm3d(self.nfb[0]),
			nn.ReLU(inplace=True))
		self.deconv3 = nn.Sequential(
			nn.ConvTranspose3d(self.nfb[0], self.nfb[1], kernel_size=2, stride=2,output_padding=(0,1,1)),
			nn.InstanceNorm3d(self.nfb[1]),
			nn.ReLU(inplace=True))
		self.deconv2 = nn.Sequential(
			nn.ConvTranspose3d(self.nfb[1], self.nfb[2], kernel_size=2, stride=2,output_padding=(0,1,1)),
